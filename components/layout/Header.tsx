        ...
        <nav className="hidden lg:flex items-center gap-6 bg-white/10 backdrop-blur-md px-5 py-2 rounded-full border border-white/15 text-xs font-medium">
          ...
          >
            <button
              type="button"
              className={cn(
                'flex items-center gap-1 transition-colors hover:text-white py-1 focus:outline-none',
            ...
            >
              <span>Services</span>
              <ChevronDown className="w-3.5 h-3.5 opacity-70 group-hover/dropdown:rotate-180 transition-transform" />
            </button>

            {servicesOpen && (
              <div className="absolute top-full left-0 mt-2 w-56 bg-solix-dark/95 backdrop-blur-xl border border-white/15 rounded-2xl p-2 shadow-solix-dark z-50 animate-fadeIn">
                {serviceLinks.map((sub) => (
                  <Link
                    key={sub.name}
                    href={sub.href}
                    className={cn(
                      'block px-4 py-2.5 rounded-xl text-xs font-semibold transition-colors',
