...
export function OverlappingMetrics() {
  const stats = [
    ...
    { value: '250+', label: 'Projects Completed', subtext: 'Commercial & industrial sites' },
  ];
  ...
  return (
    <div className="relative z-30 max-w-7xl mx-auto px-4 sm:px-8 -mt-16 sm:-mt-24 mb-16">
      ...
      >
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 divide-y sm:divide-y-0 sm:divide-x divide-solix-border/60">
          {stats.map((item, idx) => (
            <div
              key={item.label}
              className={cn(
...
// Helper cn function if needed locally
